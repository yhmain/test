...
window.tagcloud = (function(win, doc) { // ns
    ...
    TagCloud._getConfig = function (config) {
        var defaultConfig = {   //默认值
            fontsize: 14,       //基本字体大小, 单位px,之前为16px
            radius: 60,         //滚动半径, 单位px,之前100,,60
    ...
    TagCloud.prototype = {
        ...
        update: function () {
            ...
            for (var j = 0, len = self.items.length; j < len; j++) {
                ...
                //此处可以改变标签坐标
                self.items[j].element.style.left = self.items[j].x + (self.box.offsetWidth - self.items[j].offsetWidth) / 2-40 + "px";
